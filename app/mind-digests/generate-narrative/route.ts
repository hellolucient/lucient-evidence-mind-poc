...
import { processMindWatchtowerNarrativeSubmission } from "@/lib/review/mind-digests-page";
...
export async function POST(request: Request) {
  ...
  if (!digestId) {
    return NextResponse.redirect(
      new URL("/mind-digests?narrative_error=required_fields_missing", request.url),
      303
    );
  }

  const submission = await processMindWatchtowerNarrativeSubmission(auth, digestId);
