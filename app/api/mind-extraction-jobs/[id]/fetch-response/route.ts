...
import {
  buildFetchMindExtractionJobResponseApiResponse,
...
/** Explicit operator-triggered fetch. No automatic polling. */
export async function POST(request: NextRequest, context: RouteContext) {
  const { id } = await context.params;
  const route = `${mindExtractionJobsApiRoute(id)}/fetch-response`;
  ...
  const response = await buildFetchMindExtractionJobResponseApiResponse(
    id,
