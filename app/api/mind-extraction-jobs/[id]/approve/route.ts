...
import {
  buildApproveMindExtractionJobApiResponse,
...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  const route = `${mindExtractionJobsApiRoute(id)}/approve`;
  ...
  const response = await buildApproveMindExtractionJobApiResponse(
    id,
