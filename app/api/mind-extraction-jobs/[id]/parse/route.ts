...
import {
  buildParseMindExtractionJobApiResponse,
...
/** Explicit operator-triggered parse. Idempotent on success. */
export async function POST(request: NextRequest, context: RouteContext) {
  const { id } = await context.params;
  const route = `${mindExtractionJobsApiRoute(id)}/parse`;
  ...
  const response = await buildParseMindExtractionJobApiResponse(
    id,
