...
import { buildSourceDocumentDetailApiResponse } from "@/lib/review/source-documents-api";
...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  const route = `/api/source-documents/${id}`;
  ...
  const response = await buildSourceDocumentDetailApiResponse(id, auth);
