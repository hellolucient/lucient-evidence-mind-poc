...
import {
  buildSendMindRiskBriefJobApiResponse,
...
/** Operator-gated send. Requires review_status=approved. Dry-run when EXTERNAL_MIND_LIVE_SEND=false. */
export async function POST(request: NextRequest, context: RouteContext) {
  const { id } = await context.params;
  const route = `${mindRiskBriefJobsApiRoute(id)}/send`;
  ...
  const response = await buildSendMindRiskBriefJobApiResponse(
    id,
