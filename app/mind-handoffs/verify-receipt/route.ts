...
import { processMindHandoffReceiptVerificationSubmission } from "@/lib/review/mind-digests-page";
...
export async function POST(request: Request) {
  ...
  if (!handoffId) {
    return NextResponse.redirect(
      new URL("/mind-digests?receipt_error=required_fields_missing", request.url),
      303
    );
  }
